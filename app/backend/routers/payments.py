...
FRONTEND_URL_FALLBACK = "https://agrivo.vercel.app"
...
async def create_checkout(
    ...
    """Create a Stripe Checkout Session for a Pro/Empresa subscription plan."""
    origin = request.headers.get("origin") or FRONTEND_URL_FALLBACK
    success_url = f"{origin}/payment-success?plan={payload.plan}"
    cancel_url = f"{origin}/precios"
...
def _to_action_response(sub) -> "SubscriptionActionResponse":
    return SubscriptionActionResponse(
        subscription_status=sub.status,
        plan=sub.plan,
        cancel_at_period_end=bool(sub.cancel_at_period_end),
        subscription_end_date=(sub.subscription_end_date.isoformat() if sub.subscription_end_date else None),
    )


@router.get("/subscription/me", response_model=SubscriptionActionResponse)
async def get_my_subscription(
    current_user: UserResponse = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """The current user's subscription status — used by Precios/Dashboard to
    show what's active without trusting anything the frontend claims."""
    service = SubscriptionsService(db)
    sub = await service.get_or_create_subscription(str(current_user.id))
    return _to_action_response(sub)
...
async def cancel_subscription(
    ...
    service = SubscriptionsService(db)
    try:
        sub = await service.cancel_subscription(str(current_user.id))
        return _to_action_response(sub)
    except SubscriptionsNotConfiguredError as e:
...
async def resume_subscription(
    ...
    service = SubscriptionsService(db)
    try:
        sub = await service.resume_subscription(str(current_user.id))
        return _to_action_response(sub)
    except SubscriptionsNotConfiguredError as e:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(e))
    except ValueError as e:
...
async def change_plan(
    ...
    service = SubscriptionsService(db)
    try:
        sub = await service.change_plan(str(current_user.id), payload.plan)
        return _to_action_response(sub)
    except SubscriptionsNotConfiguredError as e:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(e))
    except ValueError as e:
...
async def stripe_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    subscriptions service or the ad-bookings service depending on what was
    purchased (metadata.purpose)."""
