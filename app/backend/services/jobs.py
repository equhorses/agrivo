...
class JobsService:
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Jobs]:
        """Create a new jobs"""
        try:
            if user_id:
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for jobs {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Jobs]:
        """Get jobs by ID (user can only see their own records)"""
        try:
            query = select(Jobs).where(Jobs.id == obj_id)
            if user_id:
                query = query.where(Jobs.user_id == user_id)
    ...
    async def get_list(
        ...
        """Get paginated list of jobss (user can only see their own records)"""
        try:
            query = select(Jobs)
            ...
            if user_id:
                query = query.where(Jobs.user_id == user_id)
                count_query = count_query.where(Jobs.user_id == user_id)
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Jobs]:
        """Update jobs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
    ...
    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete jobs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
