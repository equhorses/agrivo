...
async def purge_user_completely(db: AsyncSession, user_id: str) -> Optional[str]:
    """Permanently delete a user account and everything tied to it: job posts,
    bids, disputes, KYC verification, professional profile, reviews, messages,
    and subscriptions. Returns the deleted email, or None if the user didn't
    exist. Shared by the admin 'delete now' action and the daily purge job
    that enforces the 5-year retention window after a self-service deletion
    request."""
    ...
    await db.execute(Bids.__table__.delete().where(Bids.user_id == user_id))
    await db.execute(Disputes.__table__.delete().where(Disputes.user_id == user_id))
    await db.execute(Jobs.__table__.delete().where(Jobs.user_id == user_id))
    await db.execute(Kyc_verifications.__table__.delete().where(Kyc_verifications.user_id == user_id))
    await db.execute(Messages.__table__.delete().where(Messages.sender_id == user_id))
    await db.execute(Messages.__table__.delete().where(Messages.receiver_id == user_id))
    await db.execute(Profiles.__table__.delete().where(Profiles.user_id == user_id))
    await db.execute(Reviews.__table__.delete().where(Reviews.user_id == user_id))
    await db.execute(Subscriptions.__table__.delete().where(Subscriptions.user_id == user_id))
